...
describe('Save movie rate api tests', () => {
    ...
    it('should get 200 status code', () => {
        cy.saveMovieRateRequest({ apiUrl, apiToken, movieId, rate: 10 }).then(
            (data) => {
    ...
    it('should get 400 status code - rate is too high', () => {
        ...
        cy.saveMovieRateRequest({ apiUrl, apiToken, movieId, rate: 11 }).then(
            (data) => {
                const { status, success, status_code, status_message } = data;
    ...
    it('should get 400 status code - rate is too low', () => {
        ...
        cy.saveMovieRateRequest({ apiUrl, apiToken, movieId, rate: 0 }).then(
            (data) => {
                const { status, success, status_code, status_message } = data;
                expect(status).to.equal(400);
                expect(success).to.equal(false);
